...
const MFfloat Manifest_Math::DistanceLineLine(const MFpoint3& p1, const MFvec3 v1, const MFpoint3& p2, const MFvec3& v2)
{
	...
	if (std::fabs(det) > std::numeric_limits<MFfloat>::min())//lines are almost parallel
	{
		det = 1.0f / det;
	...
	const MFvec3 cDistPv1 = Cross(distP, v1);

	return std::sqrtf(Dot(cDistPv1, cDistPv1) / v12);
};
