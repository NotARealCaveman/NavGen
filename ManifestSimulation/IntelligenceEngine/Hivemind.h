...
namespace Manifest_Simulation
{
	...
	public:
		...
		//messaging observers & event spaces
		HivemindSystemObserver hivemindSystemObserver;		
	...
	private:					
		static constexpr HivemindSystemObservationToken hivemindSystemObservationToken
		{
			UnderlyingType(HivemindSystemMessageType::OBJECT_REGISTRATION | HivemindSystemMessageType::DESIRE_ASSIGNMENT)
		};	 
	...
	void HivemindSystemMessageProcessingFunction(std::vector<Message>& messages, Hivemind& hivemind);	
	void WorldTerrainProcessingFunction(const typename RCU<TerrainExtraction, TerrainExtractionDeleter>::Handle& handle, BlackBoard& blackBoard);
}
